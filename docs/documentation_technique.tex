...
\midrule
\endhead
Couverture en lignes & \textbf{91,3\%} & $\geq$ 80\% \\
Couverture en branches & \textbf{73\%} & $\geq$ 50\% \\
Couche Controllers (Auth) & 100\% & -- \\
Couche Controllers (Files) & 90,6\% & -- \\
Couche Controllers (Public) & 95,2\% & -- \\
Couche Middleware (RequestMetrics) & 87\% & -- \\
Couche Services & 75,5--100\% & -- \\
\bottomrule
\end{longtable}

Mesure du 13/09/2026 (413 lignes couvertes sur 452 et 95 branches sur 130, hors migrations ; 41 tests verts). R\'ef\'erence pr\'ec\'edente du 02/08/2026 : 83,2\% en lignes et 62\% en branches (339 lignes sur 407). L'endpoint \texttt{GET /api/files/me}, alors non couvert, l'est depuis l'ajout de \texttt{SecurityTests}. Le rapport HTML est g\'en\'er\'e dans \texttt{backend/TestResults/CoverageReport/index.html}.
...
Tests r\'ealis\'es avec 20 utilisateurs virtuels pendant 30 secondes, API et PostgreSQL en Docker (r\'ef\'erence du 17 f\'evrier 2026, puis run du 13 septembre 2026 sur la machine de d\'emonstration avec k6 2.2.0) :

\begin{longtable}{p{3.6cm} p{2.6cm} p{2.6cm} p{2.2cm} p{1.8cm}}
\toprule
\textbf{M\'etrique (upload 100 Ko)} & \textbf{17/02/2026} & \textbf{13/09/2026} & \textbf{Objectif} & \textbf{Statut} \\
\midrule
\endhead
It\'erations totales & 500 uploads & 600 uploads & > 100 & Pass\'e \\
Requ\^etes/seconde & $\sim$32/s & $\sim$20/s (limit\'e par le \texttt{sleep(1)} du script) & > 10/s & Pass\'e \\
Temps moyen & 107,9 ms & 9,3 ms & < 500 ms & Pass\'e \\
P95 & 125,19 ms & 17,5 ms & < 2000 ms & Pass\'e \\
Taux d'erreur & 0,00\% & 0,00\% & < 1\% & Pass\'e \\
Donn\'ees transf\'er\'ees & 52 Mo & 62 Mo & -- & Info \\
\bottomrule
\end{longtable}

Connexion (\texttt{perf/k6-login-test.js}, 13/09/2026) : 580 connexions, temps moyen 59,1 ms, \textbf{P95 78,9 ms} (objectif < 500 ms), 0,00\,\% d'erreur --- le hachage PBKDF2 explique le co\^ut sup\'erieur \`a celui d'un upload.

\textbf{Analyse :} \`a 20 utilisateurs simultan\'es, l'upload complet (validation JWT, \'ecriture disque, insertion PostgreSQL) reste sous 40~ms au maximum et autour de 10~ms en moyenne sans d\'egradation sur 30~s ; les I/O sont asynchrones (\texttt{CopyToAsync}, \texttt{SaveChangesAsync}), la limite r\'eelle est le disque et le r\'eseau, pas le code. Les scripts \texttt{perf/k6-upload-test.js} et \texttt{perf/k6-login-test.js} cr\'eent leur propre compte de test (\texttt{setup()}) et acceptent \texttt{BASE\_URL}, \texttt{VUS}, \texttt{DURATION}, \texttt{FILE\_KB} en variables d'environnement.
...
\subsubsection{R\'esultats Lighthouse (run du 13/09/2026, r\'ef\'erence du 31/03/2026)}

\begin{longtable}{p{3.5cm} p{2.2cm} p{2.2cm} p{2cm} p{2.5cm}}
\toprule
\textbf{Cat\'egorie} & \textbf{13/09/2026} & \textbf{31/03/2026} & \textbf{Objectif} & \textbf{Statut} \\
\midrule
Performance & \textbf{99} & 96 & $\geq$ 90 & Pass\'e \\
Accessibility & \textbf{88} & 76 & $\geq$ 90 & \`A am\'eliorer \\
Best Practices & \textbf{100} & 100 & $\geq$ 90 & Pass\'e \\
SEO & \textbf{91} & 82 & $\geq$ 80 & Pass\'e \\
\bottomrule
\end{longtable}

\textbf{M\'etriques d\'etaill\'ees (13/09/2026) :} FCP 1,7\,s, LCP 1,8\,s, TBT 0\,ms, CLS 0,001, Speed Index 1,7\,s (31/03/2026 : FCP 2,1\,s, LCP 2,3\,s, TBT 0\,ms, CLS 0,001, Speed Index 2,1\,s). Rapport complet : \texttt{docs/lighthouse-report.html}.

\textbf{Analyse :} Le score Accessibility est pass\'e de 76 \`a 88 gr\^ace aux corrections hors charte (attribut \texttt{lang="fr"}, titre de page, meta description) ; le seul audit encore en \'echec sur la page d'accueil est \texttt{button-name} (bouton d'ajout de fichier sans nom accessible). Les contrastes issus de la charte Figma restent remont\'es \`a l'UX designer. Le FCP (1,7\,s) d\'epasse encore le budget de 1,5\,s, optimisable via l'auto-h\'ebergement ou le pr\'echargement des polices.
...
Revue finale avant soutenance (09/2026) & Audit de coh\'erence code $\leftrightarrow$ documentation (OpenAPI p\'erim\'e, \texttt{SECURITY.md} d\'ecrivant des m\'ecanismes absents du code), propositions de tests de s\'ecurit\'e et de logs structur\'es, relecture du support ; puis session de finition assist\'ee (Claude Code) : compilation et ex\'ecution des tests des commits pr\'epar\'es (41/41), couverture remesur\'ee (91\,\%), correction d'un \'ecart code/documentation d\'etect\'e par l'audit (liste noire d'extensions non appliqu\'ee \`a l'upload anonyme), alignement d'OpenAPI et d'\texttt{API\_REFERENCE.md} sur les contr\^oleurs, Lighthouse relanc\'e. Chaque proposition relue, ex\'ecut\'ee (\texttt{dotnet test}, \texttt{npm run build}, parcours de d\'emonstration) et valid\'ee avant commit \\
\bottomrule
\end{longtable}
