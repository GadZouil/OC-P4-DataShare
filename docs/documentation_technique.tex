...
\begin{lstlisting}[title={Sch\'ema d'architecture}]
+------------------+         +------------------------+         +----------------+
|  Vue 3 SPA       |  HTTP   |  ASP.NET Core 9 API    | EF Core |  PostgreSQL 16 |
|  (TypeScript)    | ------> |  Controllers / Services | ------> |  (Docker)      |
|  Vite + Pinia    |         |  Identity + JWT         |         |                |
+------------------+         +------------------------+         +----------------+
                                       |
                                       v
                              +------------------+
...
\begin{lstlisting}[title={Diagramme des relations}]
+------------------+          +------------------+
|     AppUser      |          |     FileItem     |
|------------------|          |------------------|
...
+------------------+          | Token (Unique)   |
                              | Tags (text[])    |
     AppUser (1) -----> (N) FileItem             |
     (relation optionnelle :  | PasswordHash     |
      OwnerId nullable pour   | ...              |
      les uploads anonymes)   +------------------+
\end{lstlisting}
